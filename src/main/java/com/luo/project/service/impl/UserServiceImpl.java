...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
    ...
    public long userRegister(String userAccount, String userPassword, String checkPassword) {
        int userAccountMaxLen = 4;
        int userPasswordMaxLen = 8;
        // 1. 校验
        if (StringUtils.isAnyBlank(userAccount, userPassword, checkPassword)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "参数为空");
        }
        if (userAccount.length() < userAccountMaxLen) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "用户账号过短");
        }
        if (userPassword.length() < userPasswordMaxLen || checkPassword.length() < userPasswordMaxLen) {
    ...
    public User userLogin(String userAccount, String userPassword, HttpServletRequest request) {
        ...
        if (userPassword.length() < userPasswordMaxLen) {
